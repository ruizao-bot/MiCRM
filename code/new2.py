...
project_root = os.path.dirname(code_path)  # 项目根目录
...
# 2. 全局绘图设置
# =========================
pal_rgb = {"1": "#E74C3C", "2": "#2FCD71", "3": "#3498DB"}
...
def style_ax(ax, grid=False):
    ...
    ax.tick_params(axis="both", width=0.3, colors="black")
    if grid:
        ax.grid(True, which="major", color="#E5E5E5", linewidth=0.3)
        ax.grid(True, which="minor", color="#F2F2F2", linewidth=0.15)
    else:
        ax.grid(False)
...
# =========================
def load_main_data():
    csv_path = os.path.join(project_root, "data", "coal.csv")
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"没有找到 {csv_path}")
...
# 7. 参数计算函数
# =========================
def compute_theory_parameters(df, df_surv):
    """
    Compute mechanistic theoretical curve parameters and save to CSV.
    This function does not create plots.
    """
    print("\n=== Mechanistic theoretical curve parameters computed from full data ===")
    theory_rows = []
    ...
    for comm in ["1", "2", "3"]:
        ...
        try:
            params = estimate_theory_params_mechanistic(dat_full, survival_threshold=SURVIVAL_THRESHOLD)

            if params is None:
                raise ValueError("机制参数无法估计")
            ...
            theory_rows.append({
                ...
                "Cmax": np.nan,
                "Theory_R2_log": np.nan,
                "N_survivors": len(dat_surv),
                "N_seeds": dat_full["Seed"].nunique() if "Seed" in dat_full.columns else np.nan,
    ...
    theory_df.to_csv(os.path.join(project_root, "data", "coal_theory_curve_parameters.csv"), index=False)
    print(f"\nTheory parameters saved to: {os.path.join(project_root, 'data', 'coal_theory_curve_parameters.csv')}")
...
def main():
    ...
    df.to_csv(os.path.join(project_root, "data", "coal.csv"), index=False)
    ...
    summary_df.to_csv(os.path.join(project_root, "data", "coal_summary.csv"), index=False)
    params_df.to_csv(os.path.join(project_root, "data", "cue_abundance_theory_params.csv"), index=False)

    print("模拟完成，结果已保存：")
    print(os.path.join(project_root, "data", "coal.csv"))
    print(os.path.join(project_root, "data", "coal_summary.csv"))
    print(os.path.join(project_root, "data", "cue_abundance_theory_params.csv"))
    print(os.path.join(project_root, "data", "coal_theory_curve_parameters.csv"))
...
if __name__ == "__main__":
    main()
    df, df_surv = load_main_data()
    compute_theory_parameters(df, df_surv)