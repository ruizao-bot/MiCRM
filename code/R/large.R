...
# Data
df <- read.csv("../data/coal_sameR0_large.csv")
...
ggsave("../results/abundance_histogram_all.pdf", plot = p_hist, device = cairo_pdf, device = cairo_pdf, width = 21, height = 18, units = "cm", dpi = 600, bg = "white")
...
ggsave("../results/survival_boxplot_all.pdf", plot = p_box, device = cairo_pdf, width = 21, height = 10, units = "cm", dpi = 600, bg = "white")
...
p_logistic <- ggplot(df_surv, aes(x = Species_CUE, y = Abundance, colour = factor(Community))) +
  ...
  scale_color_manual(values = pal_rgb)
ggsave("../results/logistic_fit_all.pdf", plot = p_logistic, device = cairo_pdf, width = 21, height = 8, units = "cm", dpi = 600, bg = "white")
...
p_sim <- ggplot() +
  ...
  base_theme
ggsave("../results/similarity_all.pdf", plot = p_sim, device = cairo_pdf, width = 21, height = 12, units = "cm", dpi = 600, bg = "white")
...
# 导出高清图像（A4 宽，论文可用）
ggsave("../results/cue_abund_all.pdf",
       plot = final_plot,
       device = cairo_pdf,
       width = 21,      # A4 宽
       height = 24,     # 每个子图约 8cm
...
ggsave("../results/niche_overlap_all.pdf",
       plot = p_comp,
       device = cairo_pdf,
       width = 21,    # A4 宽
       height = 12,   # 自定义高度
...
ggsave("../results/Dominance_all.pdf",
       plot = p_domin,
       device = cairo_pdf,
       width = 21,    # A4 宽
       height = 12,   # 自定义高度
...
ggsave("../results/richness_all.pdf",
       plot = p_rich,
       device = cairo_pdf,
       width = 21,    # A4 宽
       height = 12,   # 自定义高度
