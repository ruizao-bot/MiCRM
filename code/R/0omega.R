...
# 封装一个简短函数：输入向量 → 输出拟合结果
fit_lognorm <- function(x) {
  fit <- fitdist(x, "lnorm")                               # MLE 拟合
  ks  <- ks.test(x, "plnorm",                              # KS 检验
...
results <- df_surv %>% 
  group_by(Community) %>% 
  summarise(fit_out = list(fit_lognorm(Species_CUE)), .groups = "drop") %>% 
  unnest(fit_out)
...
df_ext <- df %>% 
  ...
  dplyr::select(Community, Species_CUE, StatusGroup) %>%   # ← 用 dplyr::select
  mutate(
