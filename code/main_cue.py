...
def simulate_single_community(N, C0, N_modules, s_ratio, lambda_vec, m, seed):
    ...
    try:
        ...
        # CUE: flux-ratio definition (community-level)
        total_uptake = np.sum(u * R_eq[None, :], axis=1)        # (N,)
        net_uptake   = (1 - lambda_vec) * total_uptake - m      # (N,)
        cue = (np.sum(net_uptake[mask]) /
               (np.sum(total_uptake[mask]) + 1e-12))
...
def merge_and_measure(rep1, rep2, lambda_vec1, lambda_vec2, N_modules3=None, eps_feas=1e-3):
    try:
        ...
        # CUE: flux-ratio definition (community-level)
        total_uptake3 = np.sum(u3 * R3_eq[None, :], axis=1)          # (N3,)
        net_uptake3   = (1 - lambda_vec3) * total_uptake3 - m3        # (N3,)
        cue3 = (np.sum(net_uptake3[mask3]) /
                (np.sum(total_uptake3[mask3]) + 1e-12))

        return True, float(ev3), float(feas3), float(cue3)
    ...
    for b1, pack in bank1.items():
        for k, rep in enumerate(pack["reps"]):
            rows.append({
                ...
                "lambda_scalar": float(bank1[b1]["lambda_vec"][0])
            })
    ...
    for b1 in tqdm(balances, desc="Merging across (balance1, balance2)"):
        ...
        for b2 in balances:
            ...
            for i in range(n_pairs):
                ok, ev3, feas3, cue3 = merge_and_measure(
